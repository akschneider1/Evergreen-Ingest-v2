...
![Evergreen ecosystem architecture](Evergreen-Ingest/static/architecture.svg)
...
```bash
cp Evergreen-Ingest/.env.example Evergreen-Ingest/.env
# Add ANTHROPIC_API_KEY (default), OPENAI_API_KEY, or GOOGLE_API_KEY

pip install -r Evergreen-Ingest/requirements.txt
cd Evergreen-Ingest && python app.py
# → http://localhost:8000
```
...
pytest Evergreen-Ingest/tests/test_compare.py Evergreen-Ingest/tests/test_validate.py -v

# Full integration tests — requires OPENAI_API_KEY or GOOGLE_API_KEY
pytest Evergreen-Ingest/tests/ -v
```

The fixture documents in `Evergreen-Ingest/tests/fixtures/` contain intentional drift for both the tax and benefits domains.
